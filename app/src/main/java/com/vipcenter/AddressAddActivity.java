...
//监听选择点击事件及返回结果
    ...
    private boolean checkInput(){
        boolean isOk=true;
        if(TextUtils.isEmpty(nameEdit.getEditableText().toString().trim())){
            isOk=false;
            ToastUtil.showToast(this,"姓名不能为空");
            return isOk;
        }
        if(TextUtils.isEmpty(mobileEdit.getEditableText().toString().trim())){
            isOk=false;
        ...
        if(TextUtils.isEmpty(detailEdit.getEditableText().toString().trim())){
            isOk=false;
